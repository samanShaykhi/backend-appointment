...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    cb(null, "public/uploads/images/article/thumbnail"); // مسیر ذخیره‌سازی فایل
  },
...
const upload = multer({
  storage,
  limits: { fileSize: 2 * 1024 * 1024 }, // محدودیت حجم (۲MB)
  fileFilter,
});

// middleware آماده برای استفاده
const ImgArtUpload = (fieldName) => {
  return async (req, res, next) => {
    upload.single(fieldName)(req, res, (err) => {
      console.log(req.file)
      if (!req.file) return res.status(400).json({ message: 'مقاله پوستر ندارد' });
...
const ImgArtUploadEdite = (fieldName) => {
  return async (req, res, next) => {
    upload.single(fieldName)(req, res, (err) => {
      if (err instanceof multer.MulterError) {
        // ارورهای مخصوص multer (مثل حجم یا فرمت فایل)
...
module.exports = { ImgArtUpload, ImgArtUploadEdite };
